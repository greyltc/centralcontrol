...
class wavelabs:
    ...
    def startServer(self):
        ...
        self.server.timeout = (
            self.timeout / 10
        )  # INNER timeout when waiting for the wavelabs software to connect
    ...
    def connectToRelay(self):
        ...
        self.connection = socketserver.socket.socket(
            socketserver.socket.AF_INET, socketserver.socket.SOCK_STREAM
        )
    ...
    def startFreeFloat(
        ...
        se = ET.SubElement(
            ...
            iSeq=str(self.iseq),
            fTime=str(time),
            fIntensityRelative=str(intensity_relative),
            fIntensitySensor=str(intensity_sensor),
        )
        self.iseq = self.iseq + 1
        num_chans = len(channel_nums)
        for i in range(num_chans):
            ET.SubElement(
                se, "Channel", iCh=str(channel_nums[i]), fInt=str(channel_values[i])
            )
    ...
    def activateRecipe(self, recipe_name=None):
        ...
        if response.error != 0:
            print(
                "ERROR: Recipe '{:}' could not be activated, check that it exists".format(
                    recipe_name
                )
            )
        return response
    ...
    def waitForRunFinished(self, timeout=10000, run_ID=None):
        ...
        if run_ID == None:
            ET.SubElement(
                root, "WaitForRunFinished", iSeq=str(self.iseq), fTimeout=str(timeout)
            )
        else:
            ET.SubElement(
                root,
    ...
    def on(self):
        ...
        root = ET.Element("WLRC")
        ET.SubElement(
            root, "StartRecipe", iSeq=str(self.iseq), sAutomationID="justtext"
        )
        self.iseq = self.iseq + 1
        tree = ET.ElementTree(root)
        tree.write(self.sock_file)
...
if __name__ == "__main__":
    ...
    disco_time = 10000  # [ms]
    wl.startFreeFloat(
        time=disco_time, channel_nums=chan_names, channel_values=chan_values
    )
    n_chans = 21
