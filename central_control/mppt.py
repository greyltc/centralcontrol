  ...
  """
        ...
        if algo == "basic":
            if len(params) == 0:  #  use defaults
                ...
                if len(params) != 2:
                    raise (
                        ValueError(
                            "MPPT configuration failure, Usage: --mppt-params basic://[degrees]:[dwell]"
                        )
                    )
            ...
            if len(params) == 0:  #  use defaults
                ...
                if len(params) != 3:
                    raise (
                        ValueError(
                            "MPPT configuration failure, Usage: --mppt-params gradient_descent://[alpha]:[min_step]:[fade_in]"
                        )
                    )
                params = [float(f) for f in params]
    ...
    """
        ...
        while not abort and (run_time < duration):
            ...
            if (
                this[0] == last[0]
            ):  # do nothing if two consecutive voltages are the same (prevents div by zer below)
                pass
                # W += 1e-6 # bump the voltage by a microvolt if we couldn't sense a voltage change (prevents div by zer below)
            else:
                gradient = (loss(*this) - loss(*last)) / (
                    this[0] - last[0]
                )  # calculate the slope in the loss function
                v_step = (
                    alpha * gradient
                )  # calculate the voltage step size based on alpha and the gradient
                if (abs(v_step) < min_step) and (
                    min_step > 0
